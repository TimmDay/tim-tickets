...
- Short-lived by design, so images don't accumulate in Firestore storage: a screenshot is **deleted when its ticket moves to `done`** (drag, modal, or a merged agent PR), **is archived** (directly, or by archiving its epic), **or is deleted**. Done and archived tickets refuse new ones, and the modal hides the field for them. Replacing a screenshot overwrites the single image doc. Tickets that stay open keep theirs (at most one each, typically 150–250 KB).
- Agent runs: when the ticket has a screenshot, the dispatch payload includes `screenshotUrl` (`/api/agent/tickets/:key/screenshot`, bearer `AGENT_API_TOKEN`). The workflow downloads it into `.tim-tickets/` (the agent's git-excluded scratch folder, so never committed) and the prompt tells Claude to view it with the Read tool. A failed download logs a warning and the run continues without it.
...
- An "EH OI" ("Release the bots") button on the Current Jog board opens a confirm dialog listing eligible tickets in the jog selected in the dropdown (every ticket for "All tickets"; epic/text filters are ignored). A ticket is eligible if it's `todo` or `in_progress`, not archived, tagged `EH OI` (ignoring case and extra whitespace), and in a non-archived epic with a valid GitHub repo. When a specific jog is selected, an "Attempt tasks from backlog as well" checkbox reveals eligible tickets from other jogs; selecting these moves them into the current jog before dispatch. The checkbox is shown whenever such tickets exist (independent of whether it's ticked), and starts ticked when the selected jog has no eligible tickets of its own, so a ticket sitting in the backlog is listed rather than the dialog just saying there's nothing to do.
- **Pre-flight repo check:** GitHub accepts a dispatch (204) even when nothing in the repo listens for it, which would otherwise fail silently. So when the dialog opens it checks each target repo (`GET /api/agent-runs/repo-checks`), using the dispatch token to look for `.github/workflows/tim-tickets-agent.yml` on the default branch. If the file 404s, a lookup of the repo itself tells "workflow missing" apart from "token can't access the repo". Tickets in an unready repo are shown with the reason and can't be selected, and `POST /api/agent-runs` runs the same check and refuses them. A check that itself fails (GitHub error, rate limit, missing token) doesn't block. Limits: secrets and repo settings aren't visible to the dispatch token, and for **public** repos the token can read the workflow file even when the repo isn't granted to it, so that case surfaces as a dispatch error in the dialog's results instead.
- **Overdue reports:** if a ticket still in `todo`/`in_progress` has an `agentDispatchedAt` over 75 minutes old (the workflow's 60-minute timeout plus slack) with no report back, its card shows a "🤖 no report" badge, the ticket modal shows a banner explaining likely causes, and the dialog labels the row "dispatched … ago, no report". This catches whatever the pre-flight can't: wrong agent token, Vercel protection, cancelled runs, GitHub outages. Time-based UI renders client-side only (`useNow`) to avoid hydration mismatches.
...
- **Opus review:** each selectable row has a second checkbox, "Opus review" — independent of and disabled unless the ticket itself is selected for dispatch, and cleared whenever the ticket is deselected (individually or via "select all" / unticking it). Ticking it requests an extra, independent review pass on that ticket's PR: once the agent's workflow run opens the PR, a follow-up step runs `anthropics/claude-code-action`'s code-review plugin with Opus against it and comments directly on the PR. Purely a per-dispatch choice — nothing is persisted on the ticket.
- Confirming calls `POST /api/agent-runs`, which sends a GitHub `repository_dispatch` (`event_type: tim-tickets-agent`) to each ticket's epic repo. The payload carries the ticket key, title, body, `agentModel`, the epic's name and description, the ticket's comments (oldest first, skipping the app's own 🤖 status comments, oldest trimmed past ~20k chars), the report-back URL, and `requestOpusReview` (from the "Opus review" checkbox). Each dispatched ticket is stamped with `agentDispatchedAt`, moved from `todo` to `in_progress`, and gets a "🤖 Agent dispatched…" comment (noting when a review was requested). The dialog shows per-ticket failures (e.g. the token can't see the repo).
- Each repo runs the workflow in `docs/agent-workflow.yml`. Claude Code (via `anthropics/claude-code-action`, subscription OAuth token, the ticket's model) commits on an `agent/T-xx-<run>` branch, the workflow opens the PR, then reports back. If the ticket had a screenshot, the PR description links back to the ticket's page in tim-tickets (`/tickets/T-xx`) rather than embedding the image — GitHub's markdown renderer fetches inline images anonymously and can't pass the reviewer's session, so a plain link (opened with the reviewer's own login) is what actually works, and it avoids committing the image into the target repo. Only useful until the ticket reaches `done`, when tim-tickets deletes its own copy — fine, since review happens before merge.
- Report-back: `POST /api/agent/tickets/:key/report` with `Authorization: Bearer $AGENT_API_TOKEN` (exempt from the session-cookie gate; this is the only thing that token can do). `{outcome: 'pr_opened', prUrl}` comments the PR link and moves the ticket to `in_review`. `{outcome: 'no_changes' | 'failed', runUrl?}` just comments. `{outcome: 'already_done', reason?, runUrl?}`, sent when Claude judged everything the ticket asks for to already exist (it writes `.tim-tickets/result.json` instead of committing, per the prompt), comments "🤖 It seems these changes have already been made: <reason>" and moves the ticket to `blocked` for a human to confirm. All outcomes clear `agentDispatchedAt`.
- When you **merge** an agent PR, a second workflow (`docs/agent-merged-workflow.yml`, triggered by `pull_request_target: closed` for merged `agent/T-xx-<run>` branches) reports `{outcome: 'merged', prUrl}`. The ticket gets a "🤖 Agent PR merged" comment and moves to `done`. It reports to the repo variable `TIM_TICKETS_URL` rather than the dispatching page's URL, since a preview may be gone by merge time. Closing without merging changes nothing.
